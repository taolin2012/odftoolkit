...
public class OdfFileDom extends DocumentImpl implements NamespaceContext {
	...
	public static OdfFileDom newFileDom(OdfPackageDocument packageDocument,
		...
		// before creating a new dom, make sure that there no DOM opened for
		// this file already
		Document existingDom = packageDocument.getPackage().getCachedDom(
				packagePath);
		if (existingDom == null) {
			// ToDo: bug 264 - register OdfFileDom to this class
			if (packagePath.equals("content.xml")
					|| packagePath.endsWith("/content.xml")) {
				newFileDom = new OdfContentDom(
						(OdfSchemaDocument) packageDocument, packagePath);
			} else if (packagePath.equals("styles.xml")
					|| packagePath.endsWith("/styles.xml")) {
				newFileDom = new OdfStylesDom(
						(OdfSchemaDocument) packageDocument, packagePath);
			} else if (packagePath.equals("meta.xml")
					|| packagePath.endsWith("/meta.xml")) {
				newFileDom = new OdfMetaDom(
						(OdfSchemaDocument) packageDocument, packagePath);
			} else if (packagePath.equals("settings.xml")
					|| packagePath.endsWith("/settings.xml")) {
				newFileDom = new OdfSettingsDom(
						(OdfSchemaDocument) packageDocument, packagePath);
			} else if (packagePath.equals("META-INF/manifest.xml")
					|| packagePath.endsWith("/META-INF/manifest.xml")) {
				newFileDom = new OdfManifestDom(
						(OdfSchemaDocument) packageDocument, packagePath);
			} else {
			...
			if (existingDom instanceof OdfFileDom) {
				...
				// ToDO: Issue 264 - Otherwise if NOT an OdfFileDom serialize
				// old DOM AND CREATE A NEW ONE?!
				// Or shall we always reference to the dom, than we can not
				// inherit from Document? Pro/Con?s
				// }else{
	...
	protected void initialize() {
		...
		try {
			...
			if (fileStream != null) {
				...
				String baseUri = Util.getRDFBaseUri(mPackage.getBaseURI(),
						mPackagePath);
				sink = new JenaSink(this);
				odf.setSink(sink);
				SAXRDFaParser rdfa = SAXRDFaParser.createInstance(sink);
				rdfa.setBase(baseUri);
				// the file is parsed by ODF ContentHandler, and then RDFa
				// ContentHandler
			...
			Logger.getLogger(OdfFileDom.class.getName()).log(Level.SEVERE,
					null, ex);
		} finally {
			try {
				if (fileStream != null) {
	 ...
	 * document is usually represented as a directory with a mediatype.
	 * 
	 ...
	 */
	@Override
	public OdfElement createElementNS(String nsuri, String qname)
			throws DOMException {
	 ...
	 */
	@Override
	public OdfAttribute createAttributeNS(String nsuri, String qname)
			throws DOMException {
	...
	public String getNamespaceURI(String prefix) {
		...
		if (nsURI == null) {
			...
			Set<String> urisWithDuplicatePrefixes = this.mDuplicatePrefixesByUri
					.keySet();
	 ...
	 * exist in the XML as inner value prefixes might be used.
	 * </p>
	 * 
	 * <p>
	 ...
	 * scope in an arbitrary, <strong>implementation dependent</strong>, order</td>
	 * </tr>
	 * <tr>
	 * <td>unbound Namespace URI</td>
	 ...
	 *         the DOM, the previous prefix will be taken. In case of a given
	 *         prefix that was already registered, but related to a new URI, the
	 *         prefix will be adapted. The new prefix receives the suffix '__'
	 *         plus integer, e.g. "__1" for the first duplicate and "__2" for
	 *         the second.
	 */
	public OdfNamespace setNamespace(String prefix, String uri) {
		...
		// but save all others for the getPrefixes function. There might be
		// still some
		...
		if (existingPrefix != null) {
			...
			// Add the new prefix to the duplicate prefix map for
			...
			if (existingURI != null && !existingURI.equals(uri)) {
				...
				do {
					...
					// users have to take care for their attribute values using
					// namespace prefixes.
					prefix = prefix + "__" + i;
					i++;
		...
		// if the file Dom is already associated to parsed XML add the new
	...
	public void updateInContentMetadataCache(Node node) {
		...
		String baseUri = Util
				.getRDFBaseUri(mPackage.getBaseURI(), mPackagePath);
	...
	public Model getBookmarkRDFMetadata() {
		return BookmarkRDFMetadataExtractor.newBookmarkTextExtractor()
				.getBookmarkRDFMetadata(this);
	}
